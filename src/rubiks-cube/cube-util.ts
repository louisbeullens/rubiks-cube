...
export class CubeUtil {
  primaryEdgeAxis: TAxisName = "x";
  auxiliaryEdgeAxis: TAxisName = "y";
  cornerAxis: TAxisName = "z";
  getCubieInfoByPosition(coordinate: IPoint3D, cube?: TCube) {
    const faces = getFacesOfPoint3D(coordinate);

    const primaryIndex = faces.findIndex(
      ({ axis }) =>
        (coordinate[this.primaryEdgeAxis] && axis === this.primaryEdgeAxis) ||
        (!coordinate[this.primaryEdgeAxis] && axis === this.auxiliaryEdgeAxis)
    );
  ...
  createCubies(solvedCube: TCube, centerCubies: TCubies = rubikCubies) {
    ...
    coordinates.slice(0, 12).forEach((coordinate, i) => {
      const { primaryColor, secondaryColor } = this.getCubieInfoByPosition(
        coordinate,
        solvedCube
      );
    ...
    coordinates.slice(12, 20).forEach((coordinate, i) => {
      const { xColor, yColor, zColor } = this.getCubieInfoByPosition(
        coordinate,
        solvedCube
      );
  ...
  cubeToState(cube: TCube, cubies: TCubies = rubikCubies) {
    ...
    for (let i = 0; i < 12; i++) {
      ...
      const parity =
        (coordinate[this.primaryEdgeAxis] &&
          edgeCoordinate[this.primaryEdgeAxis]) ||
        (!coordinate[this.primaryEdgeAxis] &&
          !edgeCoordinate[this.primaryEdgeAxis])
          ? 0
          : 1;
  ...
  stateToCube(
    ...
    coordinates.slice(0, 12).forEach((coordinate, i) => {
      const { primaryFace, secondaryFace } =
        this.getCubieInfoByPosition(coordinate);
    ...
    coordinates.slice(12, 20).forEach((coordinate, i) => {
      ...
      const { xFace, yFace, zFace } = this.getCubieInfoByPosition(coordinate);
      ...
      switch (this.cornerAxis) {
        case "x": {
          cube[xFace!.faceIndex][uvToIndex(xFace!.u, xFace!.v)] = xColor;
          cube[yFace!.faceIndex][uvToIndex(yFace!.u, yFace!.v)] = parity
            ? zColor
            : yColor;
          cube[zFace!.faceIndex][uvToIndex(zFace!.u, zFace!.v)] = parity
            ? yColor
            : zColor;
          break;
        }
        case "y": {
          cube[xFace!.faceIndex][uvToIndex(xFace!.u, xFace!.v)] = parity
            ? zColor
            : xColor;
          cube[yFace!.faceIndex][uvToIndex(yFace!.u, yFace!.v)] = yColor;
          cube[zFace!.faceIndex][uvToIndex(zFace!.u, zFace!.v)] = parity
            ? xColor
            : zColor;
          break;
...
export const createCubies = cubeUtil.createCubies.bind(cubeUtil);
export const cubeToState = cubeUtil.cubeToState.bind(cubeUtil);
export const stateToCube = cubeUtil.stateToCube.bind(cubeUtil);
